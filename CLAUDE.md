...
**To start the next session:** Phase 3 — Back-testing engine + simulation engine. Back-test: FRED vintage replay against named scenarios (1970s stagflation, 2008 GFC, 2020 COVID); confirm quadrant classifier lands in expected regime with no look-ahead bias. Simulation: parameter sweep over weights, regime thresholds, and lookback windows; score each configuration against historical regime accuracy. Phase 2 Japan rollout (`config/countries/jp_bindings.yaml` + `jp_composites.yaml`) remains pending. Run `python3 -m indicators.pipeline` after BEA Q1 2026 data release to clear 3 stale US signals. EZ current account gap is unresolvable from free APIs — documented in `docs/Guidance/EU_singals_guidance.md`; Global Overview shows dash.
