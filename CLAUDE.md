...
| 2 Country rollout | 🔄 **In progress** | EZ ✅ (37) + KR ✅ (26) + JP ✅ (25) + GB ✅ (27, 2026-07-06) live; next: China (WB/IMF harmonized only) |
...
**To start the next session:** the Ray-framework roadmap (`docs/Guidance/ray_framework_roadmap.md`) is **complete through Phase G**, and Phase 2 covers 5 economies (US/EZ/GB/JP/KR). Open items, in rough priority: **China rollout** (Phase 2 order; WB/IMF harmonized series only — NBS automated pull is explicitly out of scope; expect a sparse-country build like KR), **D4** (manual-load infrastructure for V-Dem/Polity governance + GPR index), ONS registration-free API for live GB monthly CPI + e-Stat registration for JP, the pre-existing dtype test failure in `test_compare_raw_vs_processed_level_signal`, and the 2007-squeeze stage-threshold tweak candidate (needs more episodes before tuning). BEA note: `debt_service_ratio` refreshed with Q1 2026; `current_account`/`NIIP` still awaiting BEA publication (re-run pipeline when released). EZ current account gap remains unresolvable from free APIs — documented in `docs/Guidance/EU_singals_guidance.md`; Global Overview shows dash.
