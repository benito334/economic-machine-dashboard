...
| 2 Country rollout | 🔄 **In progress** | EZ ✅ (37) + KR ✅ (26) + JP ✅ (25, 2026-07-05) live; next: UK |
| 3 Back-test / regime replay | 🔄 G1+G2 done | `indicators/backtest.py` PIT replay + scenario scoring; G3 (ALFRED vintages, asset-outcome tests) pending |

**To start the next session:** follow `docs/Guidance/ray_framework_roadmap.md` — next is **Phase G3 (ALFRED vintage replay to kill data-revision look-ahead + asset-outcome predictive tests; decides rate_expectations keep/weight, the dynamic-threshold default, and stage-threshold calibration)**, then D4 (manual-load governance/GPR), UK rollout. BEA note: `debt_service_ratio` refreshed with Q1 2026; `current_account`/`NIIP` still awaiting BEA publication (re-run pipeline when released). EZ current account gap remains unresolvable from free APIs — documented in `docs/Guidance/EU_singals_guidance.md`; Global Overview shows dash.
