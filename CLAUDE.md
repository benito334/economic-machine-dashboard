...
**As of 2026-08-19:** Everything below still holds. **Digital Ray consult → 3 shipped features** (full detail: `docs/Guidance/ray_dalio_review_log.md` session 2026-08-19, `docs/worklog.md` same date). (1) **`debt_income_spread`** — new per-sector (household/corporate/government) debt-growth-vs-income-growth spread in `indicators/debt_cycle_stage.py`, computed as the YoY %-change of the EXISTING debt/GDP ratio signals (no new sourcing — a first-order identity, `%Δr ≈ %ΔDebt − %ΔGDP`), reserve-currency (US/EZ/JP) vs. other-country threshold tiers in `config/debt_cycle_stage.yaml`; warning/critical flags surfaced as a **separate** badge (not folded into Sovereign Squeeze) on Command Center, Debt Stress, and Relative Cycles; backfilled live for all 14 countries. (2) **ProdScore-vs-Growth-Z divergence read** (`_productivity_divergence()` in `dashboard/force_detail.py`, reused by Command Center's Productivity Trend card) — "Early-stage competitive advantage" / "Unsustainable-expansion watch" callouts, reusing the Growth chip's own `gz` threshold. (3) **`growth.relative_ulc`** — new signal (FRED `CCRETT02{cc}Q661N`, OECD/IMF REER unit-labor-cost-based index, verified live for 10/14 countries — CN/IN/BR/ID documented as a gap) feeding a new cross-country competitiveness ranking table on `/relative`; already trade-weighted/FX-adjusted so no cross-country normalization scheme had to be invented. Verified end-to-end via a rebuilt local Docker container (image bakes source, no bind mount) before/after on all three surfaces. US signal count 90→91. 15 new tests; suite **534 passed, zero exclusions**.

**As of 2026-07-09 (3):** Everything below still holds. (1) **Ray's dynamic regime thresholds now default ON** — `_DEFAULT_THRESHOLDS["dynamic"]=True`, the `regime-threshold-store` initial data + all 6 `.get("dynamic", …)` read-sites default True; an explicit user "off" is still respected (backtest G2: dynamic ≥ fixed). (2) **Self-contained web-traffic metrics** (`dashboard/traffic.py`, `/traffic`, no third-party tracker): every real page view appends one JSON line to `DATA_DIR/traffic.log` (append-only, never the DB); the page shows total/unique/today/7d + a 30-day bar chart + top pages. Unique visitor = per-tab sessionStorage id. Access: `TRAFFIC_KEY` set → `/traffic?key=…` (public-deploy safe, no nav link); unset + non-public → open + sidebar link. `session-id` store + a clientside generator; recording lives in `route_page`. 7 new tests; suite **461 passed, zero exclusions**. (3) **`/traffic` region breakdown** — a "Top regions" table from the visitor's browser IANA timezone (e.g. `Europe/London`), captured client-side into a `tz-region` store and logged as the `z` field (no IP, no geolocation; chosen over Google Analytics to avoid third-party data-sharing + an EU consent banner). (4) **Mobile-friendly** — added the missing viewport meta tag + a `@media (max-width:768px)` block in `theme.css` (sidebar → 46px icon rail, tighter gutters, wide tables/flex-rows scroll/wrap); verified at 390px. Suite **462 passed**.

**As of 2026-07-09 (2):** Everything below still holds. **Public/read-only mode** (`PUBLIC_MODE=1`, `dashboard/app_mode.py`) for untrusted multi-viewer/cloud deploys: per-browser settings (theme/country/windows/thresholds via localStorage) are already isolated per visitor, but three surfaces write SHARED state — the Data-updates scheduler, Weight Audit/History (YAML+DB writes), and Workbench save/delete (saved_views.json). In public mode those are hidden/disabled (Settings shows a read-only "refreshes automatically" note; Weight Audit/History nav hidden + routes return an "operator tool" notice blocking direct URLs; Workbench save/delete hidden, load kept) and their callbacks aren't registered. No defaults needed — hidden controls keep config values. Scheduler gets env-var config (`AUTO_IMPORT_ENABLED`/`AUTO_IMPORT_TIME` override schedule.json) so a headless operator configures the daily import without the UI. docker-compose exposes `PUBLIC_MODE` (charting) + `AUTO_IMPORT_*` (scheduler), all default off. Also **rebranded** to "Economic Machine Dashboard" (repo `benito334/economic-machine-dashboard`, now PUBLIC; package/paths keep `indicators_machine`). 5 new tests; suite **454 passed, zero exclusions**.

**As of 2026-07-09:** Everything below still holds. **Daily auto-import scheduler + Settings cleanup.** New opt-in feature: the dashboard can re-import all data daily at a time set in **Settings → Data updates** (toggle + time + timezone + last/next-run status + "Update now"). It automates the manual workflow — **stop charting → run pipeline → start charting** — via a new `scheduler` docker-compose service (`indicators/scheduler.py`, APScheduler) that bounces the charting container through the mounted docker socket (the DB is single-writer, so the dashboard must be down during the ~3-min import). Coordination is file-based (`indicators/schedule_config.py` → `schedule.json`/`schedule_status.json`/`run_now.trigger` in DATA_DIR), never through the DB. Degrades gracefully if docker is unavailable. `requirements.txt` += `docker`; `.env.example` += `TZ` (default America/Chicago). **Settings audit:** the Growth/Inflation/Disequilibrium window sliders were duplicated (sidebar + modal, 6 sync callbacks) — removed the modal copies (kept the sidebar ones per user), dropped the stale "re-run the pipeline" note. Opt-in (disabled by default). 9 new tests (`tests/test_scheduler.py`); suite **451 passed, zero exclusions**. Also (2026-07-09) refreshed US Q1-2026 BEA current account/NIIP + June payrolls and cleared the stale "Update now" sidebar banner; added a non-technical Dalio-framed `README.md` with page screenshots (`docs/screenshots/`); started the **Academy** side-project (`academy/` — beginner course charter + Ray-concept map).

**As of 2026-07-07 (5):** Everything below still holds. **Country coverage expanded to 14 economies, 462 signals** on a Digital Ray consult. Ray reviewed the list (log in `ray_dalio_review_log.md` Session 2026-07-07): endorsed Brazil, flagged **Switzerland marginal** + **Germany/Luxembourg borderline-redundant** with the EZ aggregate, and named his top missing economies — all **commodity/trade hubs**: Canada, Australia, Mexico, Indonesia (the set was heavy on debt-cycle pillars, light on commodity exporters; he ranked the original spec's Saudi/Russia BELOW these). User chose "the ones Ray suggested" → **built Brazil + Canada + Australia + Mexico + Indonesia** (dropped standalone Switzerland). New codes: BR/CA/AU/MX/ID (loader WB+IMF maps updated). All 5 carry BIS 3-sector credit → **all run the private/sovereign two-vote stage split**; stage reads BR/CA/MX=squeeze, AU=leveraging, ID=reflation; no gov-interest series → SOVEREIGN SQUEEZE flags can't fire. Data quirks (full detail in `data_source_wishlist.md`): BR rate=discount/Selic (no bond yield), CA richest (live IP+10y+3m+unemployment), AU quarterly CPI + no monthly IP, MX trade-only growth (no IP/no live unemployment), ID call-money rate + no IP. WB external debt fills for BR/MX/ID (EMs), null for CA/AU. Wired into all dashboards; Methodology §11/§15. 3 new tests; suite **442 passed, zero exclusions**. **Next: Ray's next tier (Vietnam/Turkey/South Africa/Saudi/Singapore; Russia = no-Rosstat constraint), or the standing D4/CPI-feed tails.** For deep-searching better/more signals (esp. sparse countries) see the new `docs/Guidance/signal_sourcing_guide.md` (gold-standard signal spec + per-country gap punch-list).
