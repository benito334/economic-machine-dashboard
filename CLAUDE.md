...
**To start the next session:** the Ray-framework roadmap (`docs/Guidance/ray_framework_roadmap.md`) is **complete through Phase G**. Open small items, in rough priority: **UK rollout** (Phase 2 order; use the JP session as the template — verify every ID first), **D4** (manual-load infrastructure for V-Dem/Polity governance + GPR index), e-Stat registration for JP monthly CPI, the pre-existing dtype test failure in `test_compare_raw_vs_processed_level_signal`, and the 2007-squeeze stage-threshold tweak candidate (needs more episodes before tuning). BEA note: `debt_service_ratio` refreshed with Q1 2026; `current_account`/`NIIP` still awaiting BEA publication (re-run pipeline when released). EZ current account gap remains unresolvable from free APIs — documented in `docs/Guidance/EU_singals_guidance.md`; Global Overview shows dash.
