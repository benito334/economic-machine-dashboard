...
- **D4 ✅ — Manual-load infrastructure built 2026-07-07:** drop-folder pattern (`MANUAL_DATA_DIR`, default `DATA_DIR/manual_data/`), `fetch_manual_series()` loader, pipeline Pass 3.8 (`provider: Manual`; missing file = pending `[SLOT]`, never a failure; malformed file fails loudly), converter scripts `scripts/prepare_vdem.py` (V-Dem CY-Core → `order.governance`, v2x_libdem, 8 countries) + `scripts/prepare_gpr.py` (`data_gpr_export.xls` → `order.geopolitical_risk`, GPRC columns, 7 countries — no LU in the GPR set), 15 Manual bindings, CC big-cycle card shows the reads when files land and names the pending slots until then. Docs: `docs/manual_data.md` (mirrored as the drop folder's README). A composite "order score" stays premature until the drops are loaded and have proven stable through a couple of refresh cycles.
