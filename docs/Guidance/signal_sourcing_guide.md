...
| **UK** | Rich | ✅ **Monthly CPI CLOSED 2026-07-09** — live via ONS (`fetch_ons_series`, provider `ONS`, series_id `d7g7/mm23`, to 2026-05 = 2.8%); inflation force C→B. Remaining: growth 4/4 stale (retail/IP aging); single rate + single credit signal; no daily FTSE; no DSR | **Next ONS adds** (`fetch_ons_series` is now topic-agnostic): retail sales `j5ek/drsi` + total IP (verify CDID) + a monthly unemployment CDID → would likely tip GB to B |
| **Japan** | Rich, TRUE daily vol | **Monthly CPI dead since 2021-06 → IMF ANNUAL bridge only** (worst CPI staleness); no DSR | **e-Stat API (needs free registration)** for live monthly CPI — the top JP find |
| **South Korea** | Moderate | Monthly CPI dead 2025-04 → IMF bridge; single rate signal; monthly-proxy vol; no DSR | BoK ECOS API (registration); a daily KOSPI |
...
| **Brazil** | Rich (live IP, BIS 3-sector, ext debt) | **No OECD bond yield → discount rate (Selic-linked) proxies the rate**; monthly CPI dead 2025-04 → bridge; annual unemployment proxy; monthly-proxy vol; no DSR | A free Brazilian 10y (BCB SGS API?); IBGE PNAD monthly unemployment |
