...
| **Indonesia** | Sparse — ✅ **upgraded C→B 2026-07-10** | ✅ **CPI CLOSED** — live monthly via BPS WebAPI (`fetch_bps_series`, provider `BPS`, var `2245` = CPI 150-regency general 2022=100, national INDONESIA row; needs `BPS_KEY`, short history from 2024), YoY to 2026-06 = 3.3%. Inflation C→B, **overall B (73)**. Remaining: no bond yield (call-money proxy); trade-only growth (4/5 stale/proxy); no DSR | BPS IP/trade + a BI bond yield; older CPI base to extend history |
