...
| **Japan** | Rich, TRUE daily vol | **Monthly CPI dead since 2021-06 → IMF ANNUAL bridge only** (worst CPI staleness); no DSR | **e-Stat API — https://www.e-stat.go.jp/api/en (needs free registration → `ESTAT_APP_ID`)** for live monthly CPI — the top JP find |
