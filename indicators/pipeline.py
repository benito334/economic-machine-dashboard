...
def run_country(
    ...
    # ── Pass 1.9: Brazil BCB SGS series (open, no key) ────────────────────
    if bcb_bindings:
        print(f"\n─── Pass 1.9: BCB series [{country_code}] ────────────────────────────")
        for binding in bcb_bindings:
            try:
                raw = fetch_bcb_series(binding.series_id, frequency=binding.frequency,
                                       force_refresh=force_refresh)
                if raw is None or raw.empty:
                    print(f"  [EMPTY] {binding.id} ({binding.series_id})")
                    results["empty"] += 1
